...
export default function AcceptAgreement({
  ...
  const submit = () => {
    ...
    start(async () => {
      try {
        const res = await acceptAgreement(deal.id, name, signature);
        if (!res.ok) {
      ...
      >
        <div className="space-y-4">
          ...
          <p className="rounded-lg border border-ink-600 bg-ink-900/60 p-3 text-[11px] leading-relaxed text-bone-300">
            ...
            agreement. Your name, signature, date and time are recorded against your account as
            evidence of acceptance. You will receive a copy by email immediately.
          </p>
        </div>
      </Modal>
    </>
