...
function AdminLoginForm() {
  ...
  useEffect(() => {
    ...
    if (searchParams.get("deactivated") === "1") {
      toast.error("This account is currently inactive or deactivated. Please contact your agency administrator.");
    }
    ...
    <form action={action} className="card space-y-4 p-7 shadow-2xl border-ink-600">
      ...
      {isExpired && (
        <div className="rounded-lg border border-amber-500/40 bg-amber-500/10 p-3 text-left">
          ...
          <p className="text-xs text-bone-200 leading-relaxed">
            Your previous login session has timed out. Please enter your credentials to return to the control center.
          </p>
        </div>
      )}
      ...
      {isDeactivated && (
        <div className="rounded-lg border border-rose-500/40 bg-rose-500/10 p-3.5 text-left">
          ...
          <p className="text-xs text-bone-200 leading-relaxed">
            This profile has been marked as inactive. Access to the agency control center is revoked. If you believe this is an error, please reach out to the agency owner.
          </p>
        </div>
      )}
